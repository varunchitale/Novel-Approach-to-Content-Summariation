...
class Reduction:
    ...
    def findWeight(self, sentence1, sentence2):
        l1 = list(filter(lambda w: w.Type == WordType.Content, sentence1.Words))
        l2 = list(filter(lambda w: w.Type == WordType.Content, sentence2.Words))
        length1=len(l1)
        length2=len(l2)
    ...
    def reduce(self, text, reductionRatio):
        ...
        stopWordsFile = os.path.join(dir, 'stopWords.txt')
